...
class GenomicRegion:
  ...
  *Keyword arguments:*
      ...
      - w_return -- If TRUE, it creates a new GenomicRegionSet; if FALSE, it extends self
  """
  if w_return:
   z = GenomicRegion(chrom=self.chrom, initial=self.initial-left, final=self.final+right, name=self, orientation=self.orientation, data=self.data)
