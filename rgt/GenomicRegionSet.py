   ...
   for coord in coord_dict[chrName]:
    ...
    while (counter < len(assocDict[chrName])):
     ...
     if (check == 0):  # If contain overlap, then check if the coordinate also contains overlap with the next gene
      ...
      if (
       counter < len(assocDict[chrName]) - 1):  # If this is not the last gene (there is a 'next') then verify overlap
       ...
       if (checkNext == 0):
        genesList.append(
         assocDict[chrName][counter + 1][2] + "_PROX")  # If there is an overlap then add this gene to association list
        if show_dis:
      ...
      if (maxCheck == 0):
       genesList.append(
        assocDict[chrName][counter][2] + "_DIST")  # If it overlapped then put current gene in overlap list
       if show_dis:
        dis1 = abs(coord[0] - assocDict[chrName][counter][1])
        dis2 = abs(coord[1] - assocDict[chrName][counter][0])
        if coord[1] < assocDict[chrName][counter][0]:
      ...
      if (counter > 0):  # Do this verification only if this is not the first gene
       if (assocDict[chrName][counter - 1][4] == "+"):
        geneCoordPrev = [assocDict[chrName][counter - 1][0] - promoterLength, assocDict[chrName][counter - 1][1]]
       else:
        geneCoordPrev = [assocDict[chrName][counter - 1][0], assocDict[chrName][counter - 1][1] + promoterLength]
       ...
       if (maxCheckPrev == 0):
        genesList.append(
         assocDict[chrName][counter - 1][2] + "_DIST")  # If it overlapped then put previous gene in overlap list
        if show_dis:
         dis1 = abs(coord[0] - assocDict[chrName][counter - 1][1])
         dis2 = abs(coord[1] - assocDict[chrName][counter - 1][0])
         if coord[1] < assocDict[chrName][counter - 1][0]:
